...
void UCCGameLobbyUI::UpdatePlayersList(const TArray<FLobbyPlayersData>& AllPlayers)
{
    ...
    for (FLobbyPlayersData Item : AllPlayers)
    {
        ...
        if (bShowButton && GetOwningPlayerPawn()->GetPlayerState()->GetUniqueId() == Item.NetId)
            bShowButton = false;
